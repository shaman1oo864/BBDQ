...
timeframe = '15m'			# Candle interval. One of the values 1m, 5m, 15m, 30m only this version.
tf = 15					# tf = timeframe ( 15 = '15m' )
...
since = TS - ( period * tf * 60 * 1000) # Period * tf * seconds * milliseconds
...
while True:
	...
	time.sleep(20)
	if current_time.minute % tf == 0 and current_time.strftime("%Y-%m-%d %H:%M") != last_time:
